...
<div class="container py-4">
    <h4 class="mb-4">Manage Data Processing & Storage Policy</h4>
    ...
    <form id="manageForm">
        ...
        <button class="btn btn-primary mt-3" id="formBtn">Add</button>
        ...
        <tbody>
            @foreach($result as $res)
                <tr id="row-{{ $res->id }}">
                    ...
                    <td>{{ $res->section_title }}</td>
                    <td>
                        <ul class="mb-0">
                            @foreach(explode("\n", $res->content) as $point)
                                <li>{{ trim($point) }}</li>
                            @endforeach
                        </ul>
                    </td>
                    ...
                    <td>
                        <button type="button" class="btn btn-sm btn-primary me-1 editTcBtn"
                            data-id="{{ $res->id }}"
                            data-title="{{ $res->section_title }}"
                            data-content="{{ $res->content }}"
                            data-status="{{ $res->status }}"
                        >Edit</button>
                        <button class="btn btn-sm btn-danger" onclick="deleteTC({{ $res->id }})">Delete</button>
                    </td>
                </tr>
            @endforeach
        </tbody>
...
$(document).ready(function () {
    $('#manageForm').on('submit', function (e) {
        ...
        const isUpdate = $('#data_id').val() !== '';

        $.ajax({
            url: "{{ route('admin.data-processing.store') }}",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
            ...
            success: function (data) {
                if (data.status === 1) {
                    ...
                    let points = data.res.content.split("\n").map(p => `<li>${p.trim()}</li>`).join("");
                    let row = `
                        <tr id="row-${data.res.id}">
                            <td></td>
                            <td>${data.res.section_title}</td>
                            <td><ul class="mb-0">${points}</ul></td>
                            <td>${data.res.status}</td>
                            <td>
                                <button type="button" class="btn btn-sm btn-primary me-1 editTcBtn"
                                    data-id="${data.res.id}"
                                    data-title="${data.res.section_title}"
                                    data-content="${data.res.content}"
                                    data-status="${data.res.status}"
                                >Edit</button>
                                <button class="btn btn-sm btn-danger" onclick="deleteTC(${data.res.id})">Delete</button>
                            </td>
                        </tr>
                    `;

                    if (isUpdate) {
                        $(`#row-${data.res.id}`).replaceWith(row);
                    } else {
    ...
    $(document).on('click', '.editTcBtn', function () {
        ...
        $('html, body').animate({
            scrollTop: $('#manageForm').offset().top - 100
        }, 400);
    });
});
