      ...
      <div class="container-xxl">
         ...
         <div class="card">
            <div class="card-header position-relative">
               <h6 class="fs-17 fw-semi-bold my-1">Service Providers for Plan: {{ $plan->name ?? 'All Plans' }} </h6>
            </div>
            <div class="card-body">
                     ...
                     <tbody>
                           @forelse($providers as $key => $provider)
                              <tr>
                                    ...
                                    {{-- Plan --}}
                                    <td>{{ $provider->active_subscription->plan->name ?? 'N/A' }}</td>
                              ...
                              <td>
                              @php
                                 $endDate = \Carbon\Carbon::parse($provider->active_subscription->end_date ?? null);
                                 $status = $endDate && $endDate->isFuture() ? 'Active' : 'Expired';
                              @endphp
      ...
      buttons: [
         {
               extend: 'csvHtml5',
               text: 'Download CSV',
               title: 'Service_Providers_Export',
               exportOptions: {
